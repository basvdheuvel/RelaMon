...
async function forwardMessage(message, p) {
	...
	requests.push(
		axios.post(participants[currentState["to"]], message).catch((err) => {
			...
			// Communication with other monitors cannot happen, even after 3 retries. Quit the process.
			process.exit(-1);
		})
	);
	...
	if (currentState["actionType"] == "END") {
		// Protocol finished, safe to quit the monitor
		console.log("Protocol finalised. Shutting down monitor.");
		// Wait for all requests to settle, such that all the other monitors have received the intended messages
...
// It checks whether the monitor is supposed to receive a label/value, and moves on to forward it to the
// actual receivers
function messageReceived(message, p) {
	...
	if (message["sender"] != currentState["from"]) {
		...
		throw `The received message from ${
			message["sender"]
		} contains a ${typeof message["payload"]}, while the monitor expected a ${
...
// informing the other monitors and exiting the process
async function panic(error, qs, participants) {
	console.log(error);
	// Inform the other monitors of the protocol violation
	for (let i = 0; i < qs.length; i++) {
		await axios.post(participants[qs[i]] + "/api/violation").catch((_) => {
			// Error occurred, monitor unreachable
...
// monitor to check whether it is online or not through GET requests on the route "/api/alive".
// Monitors that fail to return an appropriate response are marked as offline, and added to the
// list of failed monitors. The function then retries all the monitors that were offline in the
// previous iteration, until all the monitors are reachable.
async function waitOnNetwork(p, qs, participants) {
	const online = {};
	while (qs.length > 0) {
		// Keep track of the monitors that fail to return an appropriate response in the current iteration
		const failed = [];
		// Poll every monitor
		for (let i = 0; i < qs.length; i++) {
			await axios
				...
				.then((_) => {
					// Monitor responded, it is online.
					console.log(
						`Monitor for participant ${qs[i]} ` + chalk.green("online")
					);
				})
				.catch((_) => {
					// Error occurred, monitor unreachable
					if (!(qs[i] in online)) {
						// Only log that the monitor is offline if this is the first attempt to contact it
						console.log(
							`Monitor for participant ${qs[i]} ` + chalk.red("not online")
						);
						online[qs[i]] = false;
					}
					// Add the current monitor to the list of failed monitors
					failed.push(qs[i]);
				});
		}
		// In the next iteration, retry just the monitors that failed to respond. If all
		// monitors responded successfully, the list will be empty and the loop will stop.
		qs = failed;
	}
	networkOnline = true;
...
// Function that initialises a monitor. It reads the protocol, checks the global type for errors, checks for
...
function initialise(protocolPath) {
	...
	// Obtain the finite state machine corresponding to the given monitor process
	...
	app.post("/api/violation", (_, res) => {
		// Error route for violations at different monitors
		res.end();
		console.log("PROTOCOL VIOLATION");
		axios
			.post(participants[p] + "/api/violation")
			.catch((_) => {
				// Inform the monitor's participant of the violation
				console.log(`Error occurred when communicating with ${qs[i]}`);
				process.exit(-1);
			})
			.then((_) => {
				// Terminate the monitor's execution
				process.exit(-1);
			});
	});
	app.listen(protocol["monitorPort"], () => {
		console.log(`monitor for ${p} listening on port ${protocol["monitorPort"]}`);
	});
	// Check whether implementing party is online
	checkParty(p, [...qs], participants);
}

// Initialise the monitor with the protocol given as an argument
